...
const elcita_pay_url = 'https://electroniccity.rbccps.org:8443/api/1.0.0/payment'
...
//console.log('curl -k -XGET "'+elcita_pay_url+'" -H \'trackingid: ' + query.tracking_id + '\' -H \'apikey: 2yV42DThBljdyrm4d6qUHUmglooWyrIB\'')
var child = exec('curl -k -XGET "'+elcita_pay_url+'" -H \'trackingid: ' + query.tracking_id + '\' -H \'apikey: 2yV42DThBljdyrm4d6qUHUmglooWyrIB\'',
    (error, stdout, stderr) => {
        console.log(`stdout: ${stdout}`);
        console.log(`stderr: ${stderr}`);
res.json({'status':'OK','value':stdout, 'url':elcita_pay_url})
        if (error !== null) {
            console.log(`exec error: ${error}`);
res.json({'status':'ERROR','value':error, 'url':elcita_pay_url})
        }
});
