...
const elcita_pay_req_url = 'https://electroniccity.rbccps.org:8443/api/1.0.0/payment'
...
var child = exec('curl -k -XPOST "https://electroniccity.rbccps.org:8443/api/1.0.0/payment" -H \'email: sc.rbccps@gmail.com\' -H \'contact: 8758599133\' -H \'apikey: 2yV42DThBljdyrm4d6qUHUmglooWyrIB\' ',
    (error, stdout, stderr) => {
        console.log(`stdout: ${stdout}`);
        console.log(`stderr: ${stderr}`);
res.json({'status':'OK','value':stdout, 'url':elcita_pay_req_url})
        if (error !== null) {
            console.log(`exec error: ${error}`);
res.json({'status':'ERROR','value':error, 'url':elcita_pay_req_url})
        }
});
